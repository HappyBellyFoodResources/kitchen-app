...
class CalculateAmountWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    double deliveryCharge =
        orderController.orderDetails.order.deliveryCharge?.toDouble() ?? 0;
    double couponDiscount =
        orderController.orderDetails.order.couponDiscountAmount?.toDouble() ??
            0;
    double extraDiscount =
        orderController.orderDetails.order.extraDiscount?.toDouble() ?? 0;
    double itemsPrice = 0;
    double discount = 0;
    double tax = 0;
    double addOns = 0;
    ...
    double total = (itemsPrice + addOns + tax + deliveryCharge) -
        (discount + couponDiscount + extraDiscount);
